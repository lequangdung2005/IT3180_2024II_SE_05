...
public class TemporaryStayAbsentView extends VBox {

    private ClientSceneController sceneController;
    private List <Node> familyList;
    private VBox con;

    public TemporaryStayAbsentView (Stage stage, ClientSceneController sceneController)
    {
    ...
    private void fetchData (ClientSceneController sceneController, UpdateMessageInterface caller) {
        ...
        caller.updateMsg ("Fetching info ...");

        String username = sceneController.getClientModel().getUsername ();
        ClientMessageBase req = new ClientMessageBase (username); 
        ServerResponseTemporaryStayAbsentRequest res = sceneController.queryTemporaryStayAbsentRequest (req);
